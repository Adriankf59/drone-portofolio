...
const HomePage = () => {
  ...
  const portfolios = [
    ...
    { title: 'Bridge Inspection', description: 'A thorough inspection of a major bridge using advanced techniques.', image: '/images/brige.jpg' },
  ...
  return (
    <div className="bg-white min-h-screen font-sans">
      {/* Ensure the correct use of Header/Footer and section tags */}
      <Header />
      ...
      <section className="max-w-7xl mx-auto my-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {currentPortfolios.map((portfolio) => (
            <motion.div key={portfolio.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex flex-col p-4 shadow-lg rounded bg-white cursor-pointer border-2 transition-transform transform hover:scale-105 hover:shadow-xl border-transparent">
              <div className="overflow-hidden rounded">
        ...
        <div className="flex justify-between mt-4">
          <motion.button whileHover={{ scale: 1.1, color: "#ffffff", backgroundColor: "#4b5563" }} onClick={handlePrevPage} className="flex items-center px-4 py-2 bg-gray-300 rounded transition-all duration-200 disabled:opacity-50" disabled={portfolioPage === 0}>
            <ArrowLeftIcon className="h-6 w-6 mr-2" />
            Previous
          </motion.button>
          <motion.button whileHover={{ scale: 1.1, color: "#ffffff", backgroundColor: "#4b5563" }} onClick={handleNextPage} className="flex items-center px-4 py-2 bg-gray-300 rounded transition-all duration-200 disabled:opacity-50" disabled={(portfolioPage + 1) * pageSize >= portfolios.length}>
            Next
